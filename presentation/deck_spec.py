...
def get_slides() -> tuple[SlideSpec, ...]:
    ...
    return (
        ...
        SlideSpec(
            id="s14-regime-map",
            section="Results",
            title="Only a few sweep families create a meaningful FFT/MUSIC gap",
            takeaway="This summary is more useful than the old regime split because it isolates which sweep families actually separate FFT and MUSIC once range, velocity, and angle are averaged together.",
            visible_copy=(
                "Angle separation is the strongest FFT-favorable family.",
                "Slow-time, range separation, and fragmentation are the clearest MUSIC-favorable families.",
                "Allocation and occupancy sit near zero, so not every sweep family matters equally.",
            ),
            visuals=("story_regime_map",),
            speaker_notes=(
                "Say explicitly that this replaces the more complicated scene-by-family heat map.",
                "Use the sign of each bar to identify where FFT or MUSIC really separates after averaging over the axis-specific metrics.",
                "The point is concentration: only a few sweep families materially move the gap.",
            ),
            sources=("repo_report",),
            layout="figure_focus",
            metadata={"figure_caption": "Average MUSIC-minus-FFT delta by sweep family"},
        ),
        SlideSpec(
            id="s15-model-order",
            section="Analysis",
        ...
        SlideSpec(
            id="s16-rack-failure",
            section="Analysis",
            title="Rack aisle is a clutter-limited regime",
            takeaway="Reflective aisle geometry creates repeatable nuisance branches, so the staged search is already compromised before range/Doppler refinement begins.",
            visible_copy=(
                "The bottleneck is branch survival, not peak picking.",
                "Structured clutter behaves like a stable competitor.",
                "Practical ISAC needs clutter-aware screening before super-resolution helps.",
            ),
            visuals=("story_rack_aisle_diagnostic",),
            speaker_notes=(
                "Start with the left panel: several nuisance and alias branches survive almost every trial, while each truth sector survives in only half.",
                "Then move to the right panel: the left-rack branch survives from the candidate set into 50 of 64 final output pairs.",
                "Frame this as a clutter-limited regime boundary, not as a tunable order-selection miss.",
            ),
            sources=("repo_stage_diag", "repo_report"),
            layout="figure_focus",
            metadata={
                "figure_caption": "Rack-aisle branch survival at azimuth stage and in the final output pair",
                "panel_title": "ISAC takeaway",
            },
        ),
        SlideSpec(
            id="s17-thesis",
