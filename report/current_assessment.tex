...
The project compares a staged masked MUSIC estimator (with spatial FBSS and support-aware range/Doppler FBSS) against a fair masked FFT baseline with local refinement, both operating on the same de-embedded OFDM observation. The evidence bundle is a saved FR1 Monte Carlo study with 64 trials per sweep point across three indoor industrial scene classes and nine public sweep families including a controlled nuisance-strength study.

The main findings are: (1)~MUSIC retains real super-resolution capability in favorable regimes, winning strongly in the intersection scene and in support-limited sweeps; (2)~model-order estimation via MDL is a primary failure mechanism, with oracle known-order MUSIC recovering substantial performance in two of three scenes; (3)~in clutter-dominated regimes such as rack aisle, the failure occurs at the azimuth-stage candidate selection level before model-order even matters, as nuisance scatterers capture the spatial subspace; (4)~the resolution-versus-robustness tradeoff is the central story, not a generic superiority claim.

The project therefore positions itself as a study of where and why MUSIC super-resolution helps in realistic OFDM ISAC, identifying the adaptations needed (model-order control, clutter awareness) and the regime boundaries beyond which subspace methods lose their advantage.
\end{abstract}
...
Table~\ref{tab:model-order} summarizes the nominal comparison from confirmation runs at submission-density settings.
...
\midrule
Open aisle & 0.563 & 0.750 & 0.719 \\
Intersection & 0.703 & 0.750 & 0.156 \\
Rack aisle & 0.000 & 0.000 & 0.031 \\
\bottomrule
\end{tabular}
\end{table}

The result is decisive for two of three scenes. In open aisle, the original nominal loss to FFT ($\pjoint = 0.563$ vs $0.719$) was largely a model-order artifact: expected-order MUSIC recovers to $0.750$, matching or exceeding FFT. In intersection, expected-order MUSIC strengthens an already-clear win. In rack aisle, fixing $K=2$ does not help at all, confirming that the failure mechanism is different.

\subsection{Azimuth-Stage Failure Analysis}

The stage diagnostics logged during the model-order experiments reveal why rack aisle resists the fix. In rack-aisle trials with expected-order MUSIC, the azimuth-stage MUSIC pseudospectrum still places a persistent ghost candidate near $-21.7^\circ$, which is close to the left-rack nuisance scatterer at $-24^\circ$. The true second target near $+5.2^\circ$ never appears in the azimuth candidate pool. This means the failure is at the spatial subspace level: nuisance scatterers with gains near $-12$~dB (comparable to target strength at the scene SNR) capture the signal subspace before the staged search begins.

This is a structurally different failure from model-order overestimation. It explains why the rack-aisle regime boundary is sharp: once clutter gains cross the threshold where nuisance paths dominate the leading eigenvalues, MUSIC cannot recover regardless of model order.

\subsection{Controlled Nuisance-Strength Study}

To verify this mechanism causally, the project includes a nuisance-gain-offset sweep that shifts all nuisance scatterer gains uniformly while holding the scene geometry, target placement, and nominal separations fixed. This is the controlled single-factor study that complements the composite-regime scene comparisons.

The sweep tests offsets from $-9$~dB (attenuated clutter) to $+9$~dB (amplified clutter) relative to the scene defaults. In favorable-regime scenes, MUSIC performance is expected to degrade gracefully as clutter strengthens. In rack aisle, performance is expected to remain at zero throughout because even the nominal clutter is already dominant.

This sweep produces the strongest causal evidence in the project: it shows whether the regime boundary between MUSIC-favorable and MUSIC-unfavorable operation is driven by clutter strength rather than by the composite scene differences that confound the nominal comparison.
...
\begin{quote}
MUSIC has real super-resolution capability in realistic OFDM ISAC, but its value is conditional on two factors: (1)~the subspace dimensioning must be correct or at least controlled, because MDL-based model-order estimation fails systematically under masked covariance; and (2)~the scene must not be dominated by strong nuisance scatterers that capture the spatial subspace before the staged search begins.
\end{quote}

This is a stronger and more useful result than a simple ``MUSIC wins'' or ``MUSIC loses'' claim. It identifies \emph{why} the super-resolution upside seen in the earlier 1-D range-only study does not always survive into realistic 3-D operation, and it points to specific adaptations (model-order control, clutter-aware subspace estimation) that could recover the advantage in more regimes.
...
\begin{itemize}
  ...
  \item MDL-based model-order estimation fails systematically under masked covariance, consistently overestimating the source count. The expected-order variant demonstrates this is correctable, but a practical system would need a robust model-order method adapted to the masked setting.
...
\begin{itemize}
  \item MUSIC retains genuine super-resolution capability in favorable regimes, decisively outperforming the FFT baseline in intersection and in multiple support-limited sweeps.
  \item The original nominal open-aisle loss was substantially a model-order artifact. Expected-order MUSIC recovers to match or exceed the FFT baseline, demonstrating that the underlying super-resolution machinery is intact once subspace dimensioning is controlled.
  \item Rack aisle represents a structurally different failure: nuisance scatterers capture the spatial subspace at the azimuth stage, and no model-order fix can recover performance. The controlled nuisance-strength sweep provides causal evidence for this regime boundary.
  \item The pilot-only diagnostic confirms that the current zero-fill receiver path does not support useful sensing under extremely sparse pilot knowledge.
\end{itemize}
